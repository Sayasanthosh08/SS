setwd("C:\\python projects")
df_p = read.csv('C:\\IPL_ball_by_ball_updated till 2024 (1).csv')
library(readxl)
...
unique(df_s[["Mustafizur Rahman"]])
unique(bat$Striker)
unique(bow$Bowler) 

# Load necessary library3
library(dplyr)
...
df_s$Mustafizur Rahman <- tolower(trimws(df_s$Mustafizur Rahman))
...
df_s <- df_s%>%
  mutate(Mustafizur Rahman = ifelse(Mustafizur Rahman %in% name_mapping$matched_player, 
                         name_mapping$Striker[match(Mustafizur Rahman, name_mapping$matched_player)], 
                         Mustafizur Rahman))
...
df_s <- df_s %>%
  left_join(name_mapping, by = c("Mustafizur Rahman" = "matched_player")) %>%
  mutate(Mustafizur Rahman = ifelse(is.na(Striker), Mustafizur Rahman, Striker)) %>%
  select(-Striker)
